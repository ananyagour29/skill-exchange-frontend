...
export default function ProfilePage() {
    ...
    try {
      ...
      if (Array.isArray(exchangeRes)) {
        ...
        const formatted: SkillExchange[] = exchangeRes.map((item: any) => {
          // Ensure safe comparison by converting both to numbers
          ...
          // Grab the correct email depending on whether you are the provider or requester
  ...
  // Action Handler: Accept or Reject Requests
  const handleStatusUpdate = async (exchangeId: number, status: "ACCEPTED" | "REJECTED") => {
    ...
    try {
      ...
      await fetchProfileAndExchanges(); // Refresh page data real-time
    } catch (error) {
                        ...
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            ...
                            <div className="flex items-center gap-2 self-end sm:self-center">
                              {/* Accept & Reject Buttons: Displayed only for INCOMING PENDING Requests */}
                              ...
                              {/* Connect via Email Button: Displayed ONLY when status is ACCEPTED and partner email exists */}
                              {/* {isAccepted && item.partnerEmail && (
                                <a
                                  href={`mailto:${item.partnerEmail}?subject=Skill Exchange Connection: ${item.requestedSkill}&body=Hi ${item.partnerName},%0D%0A%0D%0AI saw that our skill exchange request for "${item.requestedSkill}" was accepted! Let's connect to schedule our session.%0D%0A%0D%0ABest regards`}
...
{isAccepted && item.partnerEmail && (
  ...
  >
    <FiMail className="text-sm" /> Connect via Email
            ...
            <div className="py-12 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-8 text-center">
              <EmptyState
                title="Profile Not Found"
                description="Please create or complete your profile details to start exchanging skills."
              />
              <div className="mt-6 text-center">
                <Link
                  href="/edit-profile"
                  className="inline-flex items-center gap-2 rounded-xl bg-teal-600 hover:bg-teal-500 text-white px-6 py-3 font-medium transition"
                >
                  Create Profile Now
                </Link>
              </div>
