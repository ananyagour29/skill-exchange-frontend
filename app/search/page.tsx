...
export default function SearchPage() {
  ...
  const filteredSkills = useMemo(() => {
    return results.filter((skill: any) => {
      // 1. Extract every possible variation of ID and Email from the skill item structure
      const ownerId = 
        skill.user?.id ?? 
        skill.user?._id ?? 
        skill.userId ?? 
      ...
      const ownerEmail = 
        ...
        skill.userEmail ?? 
        skill.user?.email ??
        skill.userEmailAddress;

      // 2. Strict ID comparison
      if (currentUserId && ownerId && String(ownerId).trim() === String(currentUserId).trim()) {
        return false;
      }

      // 3. Strict Email comparison (case-insensitive check)
      if (currentUserEmail && ownerEmail && String(ownerEmail).toLowerCase().trim() === currentUserEmail) {
        return false;
      }
  ...
  }, [results, activeCategory, currentUserId, currentUserEmail]);
