...
export default function SearchPage() {
  ...
  // Fetch current user details on mount to filter out own skills
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        const response: any = await (userService as any).getCurrentUser?.();
        const user = response?.data || response;

        if (user) {
          const extractedId = user.id ?? user._id ?? user.userId ?? user.user_id;
          const extractedEmail = user.email ?? user.userEmail;

          if (extractedId) setCurrentUserId(String(extractedId));
          if (extractedEmail) setCurrentUserEmail(String(extractedEmail).toLowerCase().trim());
        }
      } catch (err) {
  ...
  // Safe Category & Self-Skill Filter (Client-Side - matches ID and Email strictly)
// Safe Category & Self-Skill Filter (Ultra-strict fallback)
  const filteredSkills = useMemo(() => {
    const activeUserId = localStorage.getItem("currentUserId") || localStorage.getItem("userId");
    const activeUserEmail = (localStorage.getItem("currentUserEmail") || localStorage.getItem("email") || "").toLowerCase().trim();

    return results.filter((skill: any) => {
      console.log("Checking skill:", skill, "Against User ID:", activeUserId, "Email:", activeUserEmail);

      // 1. Extract every possible variation of ID and Email from the skill item
      const ownerId = 
        skill.user?.id ?? 
        skill.user?._id ?? 
        skill.userId ?? 
        ...
        skill.userDto?.id ??
        skill.userDto?._id ??
        (typeof skill.user === "string" ? skill.user : skill.user?.id);
      ...
      const ownerEmail = 
        (skill.email ?? 
         ...
         skill.user?.email ?? 
         skill.userDto?.email ?? 
         skill.userEmailAddress ?? "").toLowerCase().trim();

      // 2. Strict ID comparison
      if (activeUserId && ownerId && String(ownerId).trim() === String(activeUserId).trim()) {
        return false; // Apni skill hai, hata do
      }

      // 3. Strict Email comparison
      if (activeUserEmail && ownerEmail && ownerEmail === activeUserEmail) {
        return false; // Apni skill hai, hata do
      }
      ...
      if (activeCategory !== "All") {
        ...
        if (skill.category.toLowerCase() !== activeCategory.toLowerCase()) {
          return false;
        }
      }
        ...
        <div className="mx-auto max-w-6xl space-y-8">
            ...
            <div className="grid gap-4 md:grid-cols-12 items-center">
              ...
              <div className="md:col-span-4 flex items-center gap-2">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-teal-600 dark:bg-teal-500 py-3.5 px-4 text-sm font-bold text-white shadow-sm hover:bg-teal-700 dark:hover:bg-teal-600 transition-all cursor-pointer"
                >
          ...
          {loading ? (
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 dark:border-slate-800 p-12 text-center bg-white/50 dark:bg-slate-900/50">
              ...
              <button
                onClick={handleClearFilters}
                className="mt-4 rounded-xl bg-teal-600 dark:bg-teal-500 px-4 py-2 text-xs font-bold text-white shadow-sm hover:bg-teal-700 dark:hover:bg-teal-600 transition-all cursor-pointer"
              >
                Reset All Filters
              </button>
            </div>
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="mt-4 flex gap-2">
                      <button
                        onClick={() => router.push(`/user/${item.userId ?? item.user?.id ?? item.user}`)}
                        className="flex-1 flex items-center justify-center gap-2 rounded-xl border border-slate-200 dark:border-slate-700 py-2.5 text-xs font-bold text-slate-700 dark:text-slate-200 hover:border-teal-500 hover:text-teal-600 dark:hover:text-teal-400 transition-all cursor-pointer"
                      >
                        <FiUser /> View Profile
                      </button>
                      <button
                        onClick={() => router.push(`/ratings?userId=${item.userId ?? item.user?.id ?? item.user}`)}
                        className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-teal-600 dark:bg-teal-500 py-2.5 text-xs font-bold text-white hover:bg-teal-700 dark:hover:bg-teal-600 transition-all shadow-xs cursor-pointer"
                      >
          ...
          {totalPages > 1 && (
            <div className="mt-8 flex items-center justify-center gap-2 pt-4">
              <button
                ...
                className="p-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-50 dark:hover:bg-slate-800 transition-all cursor-pointer"
              >
